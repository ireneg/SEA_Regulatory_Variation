...
indocpm <- cpm(y) 
# lcpm <- cpm(y, log=TRUE, prior.count=0.25) # This is giving me a segfault on snowy034, can't seem to log into any other node that has R 3.5.1 installed.
...
########################################################################################################################
### IGR NOTE 2019.04.11 - THIS IS NOT WORKING BECAUSE THE LOGCPM COMMAND GIVES A SEG FAULT FOR SOME STRANGE REASONS. ###
########################################################################################################################
...
############################################################################
### IGR NOTE 2019.04.11 - THERE ARE EASIER WAYS OF MAKING THESE FIGURES. ###
###     NOTE THAT THESE ARE STILL USING Y, NOT YFILT, NEED TO UPDATE.    ###
############################################################################
...
#   nsamples <- ncol(yFilt)
#   par(mfrow=c(1,2))
#   plot(density(lcpm[,1]), col=batch.col[yFilt$samples$batch][1], lwd=2, ylim=c(0,max(density(lcpm)$yFilt)+.1), las=2, main="", xlab="")
#   title(main="A. All genes", xlab="Log-cpm")
#   abline(v=0, lty=3)
#   for (i in 2:nsamples){
#       den <- density(lcpm[,i])
#       lines(den$x, den$y, col=batch.col[yFilt$samples$batch][i], lwd=2)
#   }

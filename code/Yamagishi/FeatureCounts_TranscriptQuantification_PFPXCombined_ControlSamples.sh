...
stardir=/data/cephfs/punim0586/kbobowik/STAR/Yamagishi_CombinedPFPX_SecondPass/Controls
...
for file in ${stardir}/STAR*.bam; do
  ...
  /data/cephfs/punim0586/kbobowik/bin/subread-1.5.3-Linux-x86_64/bin/featureCounts -T 12 -a ${genomeDir}/combined_PFalc3D7_PvivaxP01_GFF.gff -t exon -g gene_id -o ${dir}/Sumba/FeatureCounts/PFPX_Combined_Yamagishi/Controls/${sample}.txt $file --verbose -B -C
  cat ${dir}/Sumba/FeatureCounts/PFPX_Combined_Yamagishi/Controls/${sample}.txt | awk '{ print $1 "\t" $6 "\t" $7 }' | tail -n +2 | sed -e "1s~${file}~Count~" > ${dir}/Sumba/FeatureCounts/PFPX_Combined_Yamagishi/Controls/Filter_GeneLengthCount_${sample}.txt
done

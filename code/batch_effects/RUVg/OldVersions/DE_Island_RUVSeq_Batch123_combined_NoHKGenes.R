...
for (i in c(1,2,3,4,5,10)){
	...
	plotPCA(set1, col=as.numeric(batch), cex=1.2, main=i)
	# lastly, plot housekeeping genes to see if batch effect is minimized
	plotPCA(set1[hkControls,], col=as.numeric(batch), cex=1.2, main=paste(i, "housekeeping Genes", sep="\n"))
}
dev.off()
...
# We can also visualise how correction looks like with a PCA. First, turn the seqExpressionSet object into a summarized experiment
...
for (i in 1:3){
	...
	write.table(commonGenes.results,file=paste0("allCommonGenes_empVsHk",colnames(dt)[i],".txt"))
}

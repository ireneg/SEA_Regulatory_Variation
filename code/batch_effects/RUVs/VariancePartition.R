...
varPart1 <- fitExtractVarPartModel(lcpm, form1, y$samples)
...
library(pvca)
pct_threshold <- 0.6
batch.factors <- c("Age","RIN","CD8T","CD4T","NK","Bcell","Mono","Gran","Island","batch")
pvcaObj <- pvcaBatchAssess (lcpm, meta, pct_threshold) 


a=AnnotatedDataFrame(meta)
b=new("ExpressionSet", exprs=lcpm, phenoData=a)
batch.factors <- colnames(a)
pct_threshold <- 0.6
pvcaObj <- pvcaBatchAssess (b, c('Island','batch'), pct_threshold) 

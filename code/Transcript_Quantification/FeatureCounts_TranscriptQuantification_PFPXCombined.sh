...
for file in ${stardir}/uniquelyMapped*.bam; do
  ...
  cat ${dir}/Sumba/FeatureCounts/PFPX_Combined_IndoSamples/${sample}.txt | awk '{ print $1 "\t" $6 "\t" $7 }' | tail -n +2 | sed -e "1s~${file}~Count~" > ${dir}/Sumba/FeatureCounts/pFalciparum/sample_counts_allSamples/Filter_GeneLengthCount_${sample}.txt
done
